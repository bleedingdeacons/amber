 ...
 *
 * Adds an admin submenu page showing attendance records for a selected
...
class IntergroupMeetingGroupAttendanceDashboard
{
    ...
    public function renderPage(): void
    {
        ...
        if ($selectedMeetingId > 0) {
            $this->renderAttendanceTable($selectedMeetingId);
        }
    ...
    /**
     * Render the attendance table for a given intergroup meeting
     *
     * @param int $meetingId Intergroup meeting ID
     */
    private function renderAttendanceTable(int $meetingId): void
    {
        $records = $this->attendanceRepository->findByIntergroupMeeting($meetingId);

        echo '<div class="ig-attendance-table-wrap">';

        if (empty($records)) {
            echo '<p class="ig-attendance-empty">No attendance records for this meeting.</p>';
            echo '</div>';
            return;
        }
        ...
        echo '<strong>' . esc_html((string) $total) . '</strong> attendance record'
            . ($total !== 1 ? 's' : '');
        if ($proxyCount > 0) {
