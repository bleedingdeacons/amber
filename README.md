...
[![Tests](https://github.com/bleedingdeacons/amber/actions/workflows/ci.yml/badge.svg)](https://github.com/bleedingdeacons/amber/actions/workflows/ci.yml)
[![Semgrep](https://github.com/bleedingdeacons/amber/actions/workflows/semgrep.yml/badge.svg?branch=main)](https://github.com/bleedingdeacons/amber/actions/workflows/semgrep.yml)
...
![PHPStan](https://img.shields.io/badge/dynamic/yaml?url=https%3A%2F%2Fraw.githubusercontent.com%2Fbleedingdeacons%2Famber%2Fmain%2Fphpstan.neon.dist&query=%24.parameters.level&label=PHPStan&prefix=level%20&color=brightgreen)
![PHPCS](https://img.shields.io/badge/dynamic/xml?url=https%3A%2F%2Fraw.githubusercontent.com%2Fbleedingdeacons%2Famber%2Fmain%2F.phpcs.xml.dist&query=%2Fruleset%2Frule%5B1%5D%2F%40ref&label=PHPCS&color=brightgreen)
...
When the **Concordance** plugin is active, Amber's `MeetingReconciler` cross-references local Unity meetings against the national AAGBDB group listings:

- **Confident match** — same day + same start time + name similarity above threshold
- **Possible match** — same day + same start time, but names are too dissimilar
- **Local only** — present in Unity but not found nationally
- **National only** — present in AAGBDB but not found locally

The meeting dashboard widget displays a colour-coded badge for each meeting indicating its reconciliation status.
